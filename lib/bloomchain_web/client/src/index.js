...
function add_helper_messages_to_subscription_form() {
  const container = document.querySelector("#js-subscription-input")
  ...
  function get_http_params() {
    ...
    let http_params = new URLSearchParams(window.location.search)

    return {
      message: http_params.get("subscription_message"),
      status: http_params.get("status"),
    }
  }
  ...
  function get_color_by_status(status) {
    ...
    return mapping[status] || "#E02829"
  }

  const subscription_params = get_http_params()

  if (Object.is(subscription_params, null)) {
    return false
  }

  const color = get_color_by_status(subscription_params.status)
  const text = subscription_params.message

  console.log(text)

  const helper_node = document.createElement("p")
  helper_node.style.cssText = `margin-bottom: 0; color: ${color}; font-size: 12px;`
  helper_node.textContent = text

  container.appendChild(helper_node)
  return
}

document.addEventListener("DOMContentLoaded", () => {
  add_helper_messages_to_subscription_form()
