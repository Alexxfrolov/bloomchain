...
function history_get_back(event) {
  ...
  const is_same_origin = () => {
    return document.referrer.match(window.location.hostname)
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const history_back_buttons = document.querySelectorAll(".js-history-back")
  Array.from(history_back_buttons).forEach((button) =>
    button.addEventListener("click", history_get_back),
  )
})
